...
# Day 28: configurable so docker-compose can point this at the backend
# service name ("http://backend:8000") instead of localhost.
...
def render_citations(chunk_ids):
    if not chunk_ids:
        return
    footnote = "Sources: " + ", ".join(chunk_ids)
    st.caption(footnote)


def build_demo_claim_card(member_id):
    card = ClaimStatusCard(
        ...
        amount=250.00,
        notes="Processed under annual checkup benefit.",
    )
    st.markdown(card.to_markdown())


def build_demo_coverage_card(plan_name, chunk_ids):
    card = CoverageSummaryCard(
        plan_name=plan_name,
        tier="Standard",
        monthly_premium=400.00,
        covered_services=["Annual checkups", "Dental", "Emergency visits"],
        citations=chunk_ids,
    )
    st.markdown(card.to_markdown())
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        full_text = ""
        citation_ids = ["benefits.txt#chunk-3", "enrollment.txt#chunk-1"]
        try:
            payload = {
        ...
        render_citations(citation_ids)

        lowered = user_input.lower()
        if "claim" in lowered:
            build_demo_claim_card(st.session_state.member_id)
        if "coverage" in lowered or "plan" in lowered:
            build_demo_coverage_card(selected_plan, citation_ids)
